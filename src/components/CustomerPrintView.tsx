    ...
    const imagesHtml = ticket.device_images && ticket.device_images.length > 0
      ? `
        <div style="margin-top:12px;">
          <div style="font-size:11px;font-weight:600;color:${SECONDARY};margin-bottom:8px;">
            Device Images &mdash; captured ${formatDateShort(ticket.created_at)}
          </div>
      ...
      ? `<tr>
          ...
          <td style="padding:6px 12px;font-size:11px;color:#111827;">
            ${ticket.device_accessories.map(a =>
              `<span style="display:inline-block;padding:2px 8px;background:#f3f4f6;border-radius:4px;margin:2px 4px 2px 0;font-size:10px;">${a}</span>`
            ).join('')}
          </td>
        </tr>`
      : '';
        ...
        <div style="padding:12px 16px;">
          <table style="width:100%;border-collapse:collapse;">
            <tr>
              ...
              <td style="padding:6px 12px;font-size:11px;color:#111827;">${ticket.device_type}${ticket.brand ? ` &mdash; ${ticket.brand}` : ''}${ticket.model ? ` ${ticket.model}` : ''}</td>
            </tr>
            ...
            <tr>
              <td style="padding:6px 12px;font-size:11px;font-weight:600;color:#6b7280;white-space:nowrap;vertical-align:top;">Est. Cost</td>
...
<head>
  <meta charset="UTF-8">
  <title>Client Report &mdash; ${customer.customer_number}</title>
  <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700&display=swap" rel="stylesheet">
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    body{font-family:'Montserrat',-apple-system,BlinkMacSystemFont,sans-serif;color:#111827;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
    @page{size:A4;margin:15mm 12mm;}
    @media print{body{padding:0;}.no-print{display:none!important;}.page-break{page-break-before:always;}}
    @media screen{body{padding:24px;background:#f0f0f0;max-width:900px;margin:0 auto;}.print-wrap{background:#fff;padding:40px;border-radius:8px;box-shadow:0 2px 20px rgba(0,0,0,.08);}}
  </style>
</head>
<body>
  <div class="no-print" style="display:flex;gap:12px;margin-bottom:20px;justify-content:center;">
    <button onclick="window.print()" style="padding:10px 28px;background:${PRIMARY};color:#fff;border:none;border-radius:8px;font-family:inherit;font-size:14px;font-weight:600;cursor:pointer;">Print Report</button>
    <button onclick="window.close()" style="padding:10px 28px;background:#fff;color:${SECONDARY};border:2px solid #d1d5db;border-radius:8px;font-family:inherit;font-size:14px;font-weight:600;cursor:pointer;">Close</button>
  </div>
  ...
  <div class="print-wrap">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:20px;border-bottom:3px solid ${PRIMARY};margin-bottom:24px;">
      <div>
        ...
        <div style="font-size:11px;color:#9ca3af;">Generated ${generatedAt}</div>
      </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:28px;">
      ...
      <div style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
        <div style="background:${PRIMARY};padding:10px 16px;">
          <span style="font-size:12px;font-weight:700;color:#fff;letter-spacing:.5px;">PERSONAL DETAILS</span>
        </div>
        <div style="padding:14px 16px;">
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;width:110px;">Customer No.</td><td style="padding:5px 0;font-size:12px;font-weight:700;color:${PRIMARY};font-family:monospace;">${customer.customer_number}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Full Name</td><td style="padding:5px 0;font-size:11px;color:#111827;font-weight:600;">${customer.title ? customer.title + ' ' : ''}${customer.first_name} ${customer.last_name}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Email</td><td style="padding:5px 0;font-size:11px;color:#111827;">${customer.email || 'Not provided'}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Phone</td><td style="padding:5px 0;font-size:11px;color:#111827;">${customer.phone || 'Not provided'}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Gender</td><td style="padding:5px 0;font-size:11px;color:#111827;">${customer.gender || 'Not specified'}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Contact Pref.</td><td style="padding:5px 0;font-size:11px;color:#111827;text-transform:capitalize;">${customer.preferred_contact_method || 'Email'}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Referral</td><td style="padding:5px 0;font-size:11px;color:#111827;text-transform:capitalize;">${customer.referral_source || 'Not specified'}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Collection</td><td style="padding:5px 0;font-size:11px;color:#111827;">${customer.needs_collection ? 'Yes &mdash; Requested' : 'No'}</td></tr>
            <tr><td style="padding:5px 0;font-size:11px;font-weight:600;color:#6b7280;">Client Since</td><td style="padding:5px 0;font-size:11px;color:#111827;">${formatDateShort(customer.created_at)}</td></tr>
          </table>
        </div>
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:20px;">
        <div style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;flex:1;">
          <div style="background:${SECONDARY};padding:10px 16px;">
            <span style="font-size:12px;font-weight:700;color:#fff;letter-spacing:.5px;">ADDRESS</span>
          </div>
          <div style="padding:14px 16px;">
            <div style="font-size:11px;color:#111827;line-height:1.7;">${fullAddress || '<span style="color:#9ca3af;">No address on file</span>'}</div>
          </div>
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
          <div style="background:${SECONDARY};padding:10px 16px;">
            <span style="font-size:12px;font-weight:700;color:#fff;letter-spacing:.5px;">REPAIR SUMMARY</span>
          </div>
          <div style="padding:14px 16px;">
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;text-align:center;">
              <div><div style="font-size:24px;font-weight:700;color:${PRIMARY};">${tickets.length}</div><div style="font-size:10px;color:#6b7280;font-weight:600;">Total</div></div>
              <div><div style="font-size:24px;font-weight:700;color:#059669;">${completedCount}</div><div style="font-size:10px;color:#6b7280;font-weight:600;">Completed</div></div>
              <div><div style="font-size:24px;font-weight:700;color:#2563eb;">${activeCount}</div><div style="font-size:10px;color:#6b7280;font-weight:600;">Active</div></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style="margin-top:32px;padding-top:16px;border-top:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;">
      <div style="font-size:9px;color:#9ca3af;">Confidential &mdash; Computer Guardian Client Report</div>
      <div style="font-size:9px;color:#9ca3af;">${customer.customer_number} &mdash; ${customer.first_name} ${customer.last_name}</div>
    </div>
  </div>
</body>
</html>`;
...
export const CustomerPrintView: React.FC<CustomerPrintViewProps> = ({ customer, tickets, onClose }) => {
  const handlePrint = () => {
    ...
    if (!win) return;
    win.document.write(buildPrintHtml(customer, tickets));
    win.document.close();
  };

  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" style={{ fontFamily: 'Montserrat, sans-serif' }}>
        <h3 className="text-lg font-bold mb-1" style={{ color: SECONDARY }}>Print Client Report</h3>
        <p className="text-sm text-gray-500 mb-6">
          Opens a print-ready page with all personal details, address, and full repair history
          including device images for <strong>{customer.first_name} {customer.last_name}</strong>.
        </p>

        <div className="bg-gray-50 rounded-lg p-4 mb-6 border border-gray-100">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs font-semibold text-gray-500">Customer No.</span>
